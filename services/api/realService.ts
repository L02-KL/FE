...
const mapCourse = (data: any): Course => {
    return {
        ...
        completedTaskCount:
            data.completedTaskCount || data.completed_task_count || 0,
...
class RealTaskService implements ITaskService {
    async getTasks(
        ...
        if (pagination?.sortBy) {
            ...
            params.append(
                "sort",
                sortMap[pagination.sortBy] || pagination.sortBy
            );
        }
    ...
    async updateTask(id: string, data: UpdateTaskRequest): Promise<Task> {
        ...
        if (data.completed !== undefined)
            payload.status = data.completed ? "done" : "pending";
    ...
    async getDashboardData(): Promise<DashboardData> {
        try {
            ...
            const stats: DashboardStats = {
                ...
                coursesCount:
                    coursesResponse.total || coursesResponse.items?.length || 0,
